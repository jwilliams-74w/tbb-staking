...
- HELIUS: **Developer plan $49/mo (9/17/26)**. Frontend uses the keyless **Secure RPC URL** `https://maud-njww5y-fast-mainnet.helius-rpc.com` (masked, 5 RPS/IP, safe for browsers) — set as NEXT_PUBLIC_RPC_URL; NO api key in the public bundle (independent auditor flagged exposed key 9/16; fixed + **old key 7b37292f ROTATED/DELETED 9/17, verified Unauthorized**). New server-side key a71061a4-… ("Scorpionripple") used ONLY by watchdog cron + admin scripts — never put it in frontend env. If abused: rotate in dashboard, update ~/.hermes/scripts/tbb_treasury_watchdog.py.
